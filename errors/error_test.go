...
func TestWrapWithMessage(t *testing.T) {
	...
	assert.Equal(t, wrapped, err.Unwrap())
	assert.Equal(t, "hello world: wrapped error", err.Error())
}
...
func TestGroup_WithError(t *testing.T) {
	...
	errGrp = errGrp.Append(New(NotFoundCode))
...
func TestGroupOrNil(t *testing.T) {
	type args struct {
		g Group
	}
	tests := []struct {
		...
		{
			name: "no errors should return nil",
			args: args{
				g: NewGroup(""),
			},
			wantNil: true,
		...
		{
			name: "group with errors should not return nil",
			args: args{
				g: NewGroup(""),
			},
			wantNil: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Nil(t, GroupOrNil(tt.args.g), fmt.Sprintf("GroupOrNil(%v)", tt.args.g))
		})
	}
}
